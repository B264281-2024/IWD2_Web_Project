...
$csv_data = [["Sequence Name", "Motif", "Start", "End", "Score", "Database"]];
            ...
            if (preg_match('/^Motif = (\S+)/', $line, $motif_match)) {
                ...
                if ($current_start && $current_end && $current_motif) {
                    $csv_data[] = [
                        ...
                        $current_start,
                        $current_end,
                        // Add any other data like Score, Database if needed
                    ];
...
fputcsv($fp, ['Sequence Header', 'Motif', 'Start', 'End', 'Score', 'Database']);
